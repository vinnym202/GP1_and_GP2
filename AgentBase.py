...
class AgentBase:
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        '''optimizer'''
        self.act_optimizer = torch.optim.AdamW(self.act.parameters(), self.learning_rate, weight_decay=args.weight_decay)
        self.cri_optimizer = torch.optim.AdamW(self.cri.parameters(), self.learning_rate, weight_decay=args.weight_decay) \
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        for t in range(horizon_len):
            ...
            ary_state, reward, done, _, _ = env.step(ary_action)  # next_state
            if done:
    ...
    def explore_vec_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        for t in range(horizon_len):
            ...
            state, reward, done, _, _ = env.step(action)  # next_state
            actions[t] = action
            rewards[t] = reward
            dones[t] = done
