...
class Config:
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        env_args.setdefault('max_step', 100_000)  # `max_step=100_000` in default, which is a large enough value.
        self.env_name = env_args['env_name']  # the name of environment. Be used to set 'cwd'.
        self.num_envs = env_args['num_envs']  # the number of sub envs in vectorized env. `num_envs=1` in single env.
        self.max_step = env_args['max_step']  # the max step number of an episode. 'set as 100_000 in default.
        ...
        self.num_workers = 8  # rollout workers number pre GPU (adjust it to get high GPU usage)
        self.num_threads = 20  # cpu_num for pytorch, `torch.set_num_threads(self.num_threads)`
...
def build_env(env_class=None, env_args: dict = None, gpu_id: int = -1):
    ...
    env_args.setdefault('max_step', 100_000)
...
def get_gym_env_args(env, if_print: bool) -> dict:
    ...
    if if_gym_standard_env and (not hasattr(env, 'num_envs')):  # isinstance(env, gym.Env):
        ...
        max_step = getattr(env, '_max_episode_steps', 100_000)
        ...
        num_envs = getattr(env, 'num_envs', 1)
        max_step = getattr(env, 'max_step', 100_000)
    ...
    def run(self):
        ...
        while True:
            ...
            if action is None:
                ...
                state, reward, done, _, info_dict = env.step(action)
                if done:
                    state, _ = env.reset()
