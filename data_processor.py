...
from processor_alpaca import AlpacaProcessor as Alpaca
...
class DataProcessor:
    ...
    def add_cdl(self, df) -> pd.DataFrame:
        df = self.processor.add_cdl(df)
    ...
    def df_to_array(self, df, if_vix, if_cdl) -> np.array:
        price_array, tech_array, turbulence_array, date_array = self.processor.df_to_array(
            df, self.tech_indicator_list, if_vix, if_cdl
        )
