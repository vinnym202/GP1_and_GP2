...
class AlpacaProcessor:
    ...
    def download_data(
        ...
        if pd.Timedelta(time_interval).total_seconds() < pd.Timedelta('1D').total_seconds():
    ...
    def add_cdl(self, data):
        df = data.copy() # Make a copy of the input dataframe

        # Loop through each candlestick pattern and calculate it for the OHLC data
        for pattern in CDL:
            df[pattern] = getattr(talib, pattern)(
    ...
    def df_to_array(self, df, tech_indicator_list, if_vix, if_cdl):
        df = df.copy()
            ...
            if if_first_time:
                ...
                if if_cdl:
                    tech_array = np.hstack(
                        [tech_array, df[df.tic == tic][CDL].values]
                    )
                date_array = df[df.tic == tic][["timestamp"]].values
                if_first_time = False
            else:
        ...
        return price_array, tech_array, turbulence_array, date_array
    ...
    def get_trading_days(self, start, end):
        ...
        # Convert timezone-aware datetime objects to timezone-naive ones
        start_naive = start_dt.astimezone(pytz.UTC).replace(tzinfo=None)
        end_naive = end_dt.astimezone(pytz.UTC).replace(tzinfo=None)
        
        df = nyse.sessions_in_range(
            pd.Timestamp(start_naive), pd.Timestamp(end_naive)
        )
        trading_days = []
        for day in df:
